...
static void MX_TIM1_Init(void)
{
  ...
  htim1.Init.Period = LED_PERIOD_TICKS;
...
void StartLedTask()
{
  ...
  osDelay(INITIAL_DELAY);
  clearFrame();
  int complete = 1;
  osMessageQueuePut(LedInitHandle, &complete, 0U, 0U);
  uint32_t delay = FrameDelayTicks(animation->fps);
  uint32_t i = 0;
  Animation* newAnim;

  for (;;) {
    osStatus newAnimStatus = osMessageQueueGet(animationQueueHandle, &newAnim, NULL, 0U);
    if(newAnimStatus == osOK) {
      Animation_Destroy(animation);
      animation = newAnim;
      i = 0;
      delay = FrameDelayTicks(animation->fps);
    }

    Led* strip = Animation_GetFrame(animation, i++);
    WS2812_SendOnce(strip, animation->led_count);
    if (i >= animation->frame_count) i = 0;
